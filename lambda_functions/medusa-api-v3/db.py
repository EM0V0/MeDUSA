...
if not USE_MEMORY:
    ...
    def _table_with_schema(env_var: str):
        ...
        try:
            ...
            if schema:
                # Correctly identify HASH and RANGE keys
                for key in schema:
                    if key["KeyType"] == "HASH":
                        pk_attr = key["AttributeName"]
                    elif key["KeyType"] == "RANGE":
                        sk_attr = key["AttributeName"]
                
                # Fallback if loop didn't work (shouldn't happen if schema exists)
                if pk_attr == "id" and schema[0]["KeyType"] != "HASH":
                     # This case is unlikely if schema is valid
                     pass
        except Exception:
            pass
    ...
    # Hardcode schema for Tremor Analysis to avoid introspection issues
    T_TREMOR_ANALYSIS = ddb.Table(os.environ["DDB_TABLE_TREMOR_ANALYSIS"])
    TREMOR_PK_ATTR = "patient_id"
    TREMOR_SK_ATTR = "timestamp"
...
def get_tremor_analysis(patient_id: str, start_time: Optional[int] = None, end_time: Optional[int] = None, limit: int = 100) -> Tuple[List[Dict[str,Any]], int]:
    ...
    try:
        ...
        for item in items:
            ...
            # Ensure tremor_score exists
            if "tremor_score" not in item and "tremor_index" in item:
                try:
                    idx = float(item["tremor_index"])
                    # If index is 0-1, score is 0-100
                    if idx <= 1.0:
                        item["tremor_score"] = idx * 100
                    else:
                        item["tremor_score"] = idx
                except:
                    pass
            ...
            if "timestamp" in item and isinstance(item["timestamp"], str):
                try:
                    ...
                    # Handle Z and potential fractional seconds
